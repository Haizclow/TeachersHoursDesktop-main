...
public class Database {
    ...
    private static void insertTestData(Statement stmt) throws SQLException {
        ...
        stmt.executeUpdate("INSERT INTO work_records (teacher_id, date, group_id, hours, lesson_type) VALUES " +
                "(1, '" + LocalDate.now().minusDays(2) + "', 1, 4, 'Лекция'), " +
                "(1, '" + LocalDate.now().minusDays(1) + "', 2, 2, 'Практика'), " +
                "(2, '" + LocalDate.now() + "', 3, 3, 'Семинар')");
    }
    ...
    public static boolean updateTeacherHours(int teacherId, int hours) {
    ...
    private static List<String> getGroupsForTeacher(int teacherId) {
    ...
    public static boolean addTeacher(String fullName, String department, List<String> groups) {
        ...
        try (Connection conn = createConnection();
            ...
            for (String groupName : groups) {
                int groupId = getOrCreateGroup(conn, groupName.trim());
                if (groupId == -1) {
                    conn.rollback();
                    return false;
                }
    ...
    public static boolean updateWorkRecord(int recordId, int hours) {
        ...
        try (Connection conn = createConnection();
            ...
            System.err.println("Error updating work record:");
            e.printStackTrace();
            return false;
        }
    }
